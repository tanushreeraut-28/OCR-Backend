...
public interface SdsApi {
    ...
    @Operation(summary = "Get OCR Extracted Data Section 2",description = "Fetch OCR extracted data for Section 2 (Hazard Identification)")
    @ApiResponses({@ApiResponse(responseCode = "200", description = "Section 2 OCR data fetched successfully"),@ApiResponse(responseCode = "404", description = "SDS not found"),@ApiResponse(responseCode = "500", description = "Internal Server Error")})
    @GetMapping(value = "/sds/{sdsId}/ocr-data/section-2", produces = "application/json")
    ResponseEntity<OcrExtractedDataSection2ResponseDto> getOcrExtractedDataSection2(@PathVariable Long sdsId);
}